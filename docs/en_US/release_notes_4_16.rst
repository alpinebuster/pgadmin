...
This release contains a number of bug fixes and new features since the release of pgAdmin.15.
...
| `Issue #4836 <https://redmine.postgresql.org/issues/4836>`_ -  Updated the json file name from 'servers.json' to 'pgadmin/servers.json' in the container deployment section of the documentation.
...
| `Issue #4926 <https://redmine.postgresql.org/issues/4926>`_ -  Fix VPN network disconnect issue where pgAdmin hangs on expanding the Servers node.
...
| `Issue #4953 <https://redmine.postgresql.org/issues/4953>`_ -  Fix an issue where pgAdmin unable to retrieve table node if the trigger is already disabled and the user clicks on Enable All.
...
| `Issue #4978 <https://redmine.postgresql.org/issues/4978>`_ -  Fix pgAdmin failed to start issue after upgrading to version 4.15.
