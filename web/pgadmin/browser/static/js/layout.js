...
_.extend(pgBrowser, {
  ...
  restore_layout: function (
    docker, layout, defaultLayoutCallback, checkLayout = false
  ) {
    ...
    if (layout != '') {
      try {
        ...
        if(checkLayout) {
          // Check restore layout is restored pgAdmin layout successfully if not then reset layout to default pgAdmin layout.
          if ((
            docker.findPanels('properties').length == 0 ||
            docker.findPanels('browser').length == 0
          ) && defaultLayoutCallback) {
            // clear the wcDocker before reset layout.
            docker.clear();
            Notify.info(
              gettext('pgAdmin has reset the layout because the previously saved layout is invalid.'),
              null
            );
  ...
  save_lock_layout: function(op) {
    ...
    getApiInstance().put(
      ...
      Notify.pgNotifier(
        'error', error,
        gettext('Failed to save the lock layout setting.')
      );
    });
  },
