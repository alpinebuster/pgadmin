  ...
  const RUNTIME_MENUS_OPTIONS = {
    runtime : {
      ...
      submenus: {
        ...
        enter_full_screen: {
          label: gettext('Enter Full Screen'),
          name: 'enter_full_screen',
          enable: true,
          priority: 2,
          key: fullScreenKey,
          modifiers: isMac() ? `${controlKey}+ctrl` : controlKey
        },
        actual_size: {
          label: gettext('Actual Size'),
          name: 'actual_size',
          priority: 3,
          enable: true,
          key: '0',
...
export default class MainMenuFactory {
  static createMainMenus() {
    ...
    MAIN_MENUS.forEach((_menu) => {
      ...
      // NOTE：Don't add menuItems for Object menu
      // as it's menuItems get changed on tree selection.
      if (_menu.name !== 'object') {
        ...
        menuObj.getMenuItems().forEach((menuItem, index) => {
          ...
          if (menuItem.below) {
            menuObj.addMenuItem(
              MainMenuFactory.getSeparator(), index + 1
            );
          }
        });
      }
  ...
  static createMenuItem(options) {
    return new MenuItem({...options, callback: () => {
      // Some callbacks registered in 'callbacks' check and
      // call specific callback function
      if (
        options.module &&
        'callbacks' in options.module &&
        options.module.callbacks[options.callback]
      ) {
        options.module.callbacks[options.callback].apply(
          options.module,
          [options.data, pgAdmin.Browser.tree?.selected()]
        );
      } else if (
        options.module &&
        options.module[options.callback]
      ) {
        ...
        if (options.url != '#') {
          ...
          api(url_for('tools.initialize')).then(()=>{
