...
export function generateNodeUrl(
  treeNodeInfo, actionType, itemNodeData, withId, jumpAfterNode
) {
  let opURL = {
      'create': 'obj',
      'drop': 'obj',
  ...
  let nodeObj = this;
  let itemID = withId &&
    itemNodeData._type == nodeObj.type
    ? encodeURIComponent(itemNodeData._id)
    : '';
  actionType = actionType in opURL
    ? opURL[actionType]
    : actionType;
  ...
  let nodePickFunction = function(treeInfoValue) {
    return (
      treeInfoValue.priority <= jump_after_priority ||
      treeInfoValue.priority == priority
    );
  };

  return generate_url(
    pgAdmin.Browser.URL, treeNodeInfo, actionType,
    nodeObj.type, nodePickFunction, itemID
  );
}
 ...
 */
export function getNodeAjaxOptions(
  url, nodeObj, treeNodeInfo, itemNodeData,
  ...
  return new Promise((resolve, reject)=>{
    ...
    if(url) {
      if(otherParams.customGenerateUrl) {
        fullUrl = otherParams.customGenerateUrl.call(
          nodeObj, treeNodeInfo, url, itemNodeData,
          otherParams.urlWithId, otherParams.jumpAfterNode
        );
      } else {
    ...
    if (url) {
      let cacheNode =
        pgAdmin.Browser.Nodes[otherParams.cacheNode] || nodeObj;
      let cacheLevel = otherParams.cacheLevel || cacheNode.cache_level(
        treeNodeInfo, otherParams.urlWithId
      );
      /*
      ...
      * and use it.
      */
      let data = cacheNode.cache(
        nodeObj.type + '#' + url, treeNodeInfo, cacheLevel
      );
      ...
      if (_.isUndefined(data) || _.isNull(data)) {
        api.get(fullUrl, {
          ...
          otherParams.useCache && cacheNode.cache(
            nodeObj.type + '#' + url, treeNodeInfo, cacheLevel, data
          );
...
export function getNodeListById(
  ...
  return getNodeAjaxOptions(
    'nodes', nodeObj, treeNodeInfo, itemNodeData, params, transform
  );
}

/* Get the nodes list based on node name passed */
export function getNodeListByName(
  node, treeNodeInfo, itemNodeData, params = {},
  filter = () => true, postTransform = (res) => res
) {
